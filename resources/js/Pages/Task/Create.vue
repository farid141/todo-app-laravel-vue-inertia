...
function handleSubmit() {
    form[isEdit ? 'put' : 'post'](route(isEdit ? 'tasks.update' : 'tasks.store', isEdit ? props.task.id : undefined), {
        onError: (errors) => {
                ...
                <div class="bg-white shadow sm:rounded-lg sm:p-8">
                    <form @submit.prevent="handleSubmit" class="space-y-6">
                        ...
                        <div class="flex items-center gap-4">
                            <PrimaryButton :disabled="form.processing">{{ isEdit ? 'Update' : 'Save' }}</PrimaryButton>
